...
const AugmentedReality = (props: AugmentedRealityProps) => {
  ...
  const [arOn, setArOn] = React.useState<Boolean>(false);
  ...
  React.useEffect(() => {
    const updateImages = (imagesToUpdate?: GltfImage[]) => {
      ...
      const toAdd = imagesToUpdate?.filter(
        ({ key }) => !imagesInView.current.find((image) => image.key === key)
      );
  ...
  if (!support.isSupported) {
    ...
    return (
      <Unsupported
        {...otherProps}
        reason={support.unsupportedReason as UnsupportedReason}
      />
    );
  }
  ...
  const addDomOverlayOptions = (sessionOptions?: XRSessionOptions) => {
    ...
    return {
      ...sessionOptions,
      domOverlayOptions: { rootElementId: overlayElementId },
    };
  };
